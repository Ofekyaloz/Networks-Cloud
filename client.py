...
try:
    ...
    print("The server is unreachable in ", (ip, port))
...
def on_created(event):
    ...
    if os.path.isfile(event.src_path):
        ...
        msg = (
            DELIMITER.join(
                [SEND_FILE, str(file), str(size), str(event.src_path), str(client_id), computer_id])).encode(
            UTF)
...
def on_moved(event):
    ...
    if event.is_directory:
        msg = (DELIMITER.join(
            [ALERT_MOVED_FOLDER, str(event.src_path), str(event.dest_path), str(client_id), computer_id])).encode(
            UTF)
